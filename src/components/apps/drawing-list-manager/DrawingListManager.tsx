		...
		>
					...
					<button
						...
						onClick={() =>
							handleExport().catch((err) =>
								logger.error("DrawingListManager", "Export error", {
									error: err,
								}),
							)
						}
				...
				>
					...
					<div style={{ display: "grid", gap: 12, marginTop: 14 }}>
						...
						>
							...
							>
								{buildProjectCode(projectConfig.projectNumber)}-DISC-TYPE-###
								REV
							</div>
						</div>
					</div>
			...
			>
				...
				>
					...
					<div style={{ fontSize: 12, color: palette.textMuted }}>
						{filteredDrawings.length} entries
					</div>
				</div>
								...
								>
									...
									<td style={{ padding: "8px 6px" }}>
										<input
											value={drawing.title}
											onChange={(e) =>
												updateDrawingTitle(drawing.id, e.target.value)
											}
											style={{
												width: "100%",
												padding: "4px 6px",
			...
			>
				...
				<p
					style={{ margin: "6px 0 0", fontSize: 12, color: palette.textMuted }}
				>
