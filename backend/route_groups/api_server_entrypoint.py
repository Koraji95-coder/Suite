...
def run_server_entrypoint(
    *,
    ...
    debug: bool = False,
    threaded: bool = False,
    allow_dev_server: bool = True,
) -> Tuple[str, int]:
    if not allow_dev_server:
        raise RuntimeError(
            "Flask development server start is disabled. "
            "Use a production WSGI/ASGI server and import the Flask app object instead."
        )
