...
import type { ToWorkerMessage, SimConfig } from './workerTypes';
...
function buildSim(config: SimConfig) {
  ...
  sim = forceSimulation<WNode>(nodes)
    .force('link', forceLink<WNode, WLink>()
      .id(d => d.id)
      .distance(d =>
        d.type === 'orchestrator' ? config.linkDistance.orchestrator
        : d.type === 'overlap' ? config.linkDistance.overlap
        : config.linkDistance.subfeature)
      .strength(d =>
        d.type === 'orchestrator' ? config.linkStrength.orchestrator
        : d.type === 'overlap' ? config.linkStrength.overlap
        : config.linkStrength.subfeature))
    .force('charge', forceManyBody<WNode>().strength(d =>
      d.type === 'major' ? config.chargeStrength.major : config.chargeStrength.minor))
    .force('center', forceCenter(0, 0))
    .force('collision', forceCollide<WNode>().radius(d => d.r + config.collisionPadding))
...
self.onmessage = (e: MessageEvent<ToWorkerMessage>) => {
  ...
  switch (msg.type) {
    case 'init': {
      ...
      (s.force('link') as ReturnType<typeof forceLink<WNode, WLink>>).links(links);
      ...
      s.on('end', () => {
        ...
        (self as unknown as Worker).postMessage({ type: 'settled', alpha: s.alpha() });
      });
      break;
    }
