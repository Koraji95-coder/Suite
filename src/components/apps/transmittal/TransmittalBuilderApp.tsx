...
const buildPayload = (draft: DraftState, files: FileState): TransmittalPayload => {
	const contacts = draft.contacts
		.filter(isContactComplete)
		.map((contact) => ({
...
const TransmittalSection = (props: ComponentProps<typeof FrameSection>) => (
	<FrameSection {...props} specular={false} />
);
...
export function TransmittalBuilderApp() {
	...
	const removeCidDocument = (fileName: string) => {
		...
		setDraft((prev) => ({
			...prev,
			cidDocuments: prev.cidDocuments.filter((doc) => doc.fileName !== fileName),
		}));
	};
