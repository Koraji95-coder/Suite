...
export function useGroundGrid() {
  ...
  if (!ctx) throw new Error('useGroundGrid must be used within GroundGridProvider');
  return ctx;
}
...
export function GroundGridProvider({ children }: { children: React.ReactNode }) {
      ...
      try {
        ...
        if (isFirstCheck) {
          if (isNowConnected) {
            ...
            setAvailableLayers(layers);
            if (layers.length > 0) {
              addLog('system', `Retrieved ${layers.length} layers from active drawing`);
            }
          } else {
            addLog('system', 'AutoCAD backend not detected');
            addLog('system', 'Start AutoCAD and the backend server to enable CAD features');
            if (!shownDisconnectToastRef.current) {
              shownDisconnectToastRef.current = true;
              showToast('error', 'AutoCAD backend not detected. Check the Log tab for details.');
            }
          }
          ...
          if (isNowConnected) {
            ...
            if (!shownDisconnectToastRef.current) {
              shownDisconnectToastRef.current = true;
              showToast('error', 'AutoCAD connection lost. Check the Log tab for details.');
            }
          }
        }
        ...
        if (isFirstCheck) {
          addLog('system', 'AutoCAD backend not detected - features will activate when available');
          if (!shownDisconnectToastRef.current) {
            shownDisconnectToastRef.current = true;
            showToast('error', 'AutoCAD backend not detected. Check the Log tab for details.');
          }
  ...
  return (
    <GroundGridContext.Provider value={{ logs, addLog, clearLogs, backendConnected, availableLayers, refreshLayers }}>
      {children}
