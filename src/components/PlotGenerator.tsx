...
type PlotType = 'sine' | 'square' | 'sawtooth' | 'rc-charge' | 'rl-response' | 'bode';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-3">
        ...
        <h2 className="text-3xl font-bold text-white/90">Plot Diagrams</h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          {(plotType === 'sine' || plotType === 'square' || plotType === 'sawtooth') && (
      ...
      <FrameSection title="Plot Information">
        <div className="text-orange-300 space-y-2 text-sm">
          {plotType === 'sine' && (
            <p>Sinusoidal waveform commonly found in AC circuits and signal processing.</p>
          )}
          {plotType === 'square' && (
            <p>Square wave with rapid transitions, useful for digital circuits and PWM.</p>
          )}
          {plotType === 'sawtooth' && (
            <p>Linear ramp waveform used in oscilloscopes and signal generation.</p>
          )}
          {plotType === 'rc-charge' && (
            <p>Exponential charging response of an RC circuit when a voltage is applied.</p>
          )}
          {plotType === 'rl-response' && (
            <p>Exponential decay of current in an RL circuit after voltage is removed.</p>
          )}
          {plotType === 'bode' && (
            <p>Frequency response magnitude plot showing system gain vs frequency.</p>
          )}
        </div>
      </FrameSection>
    </div>
