import { GripVertical, Eye, EyeOff, RotateCcw, X, BarChart3, Calendar, Activity, FolderKanban } from 'lucide-react';
...
function SortableWidgetRow({
  ...
  return (
    ...
    >
      ...
      <Icon size={16} style={{ color: widget.visible ? palette.primary : palette.textMuted, flexShrink: 0 }} />
      <span
      ...
      <button
        ...
        style={{
          background: hexToRgba(widget.visible ? palette.primary : palette.textMuted, 0.1),
...
export function DashboardCustomizer({ widgets, onToggle, onReorder, onReset, onClose }: DashboardCustomizerProps) {
  const { palette } = useTheme();
      ...
      <div className="relative z-10">
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={widgets.map(w => w.id)} strategy={verticalListSortingStrategy}>
            {widgets.map(widget => (
              <SortableWidgetRow key={widget.id} widget={widget} onToggle={onToggle} />
            ))}
          </SortableContext>
        </DndContext>
      </div>
