...
import { normalizeAgentResponseText } from "@/components/agent/agentResponseNormalizer";
import { AgentPanelBoundary } from "@/components/agent/AgentPanelBoundary";

type TabId = "architecture" | "rules" | "pipeline" | "training" | "compare";
...
const TABS: Array<{ id: TabId; label: string; icon: string }> = [
	...
	{ id: "compare", label: "Compare", icon: "\u{2B21}" },
];
...
function extractAgentResponseText(data: Record<string, unknown> | undefined): string {
	...
	for (const key of directKeys) {
		...
		if (typeof value === "string" && value.trim()) {
			return normalizeAgentResponseText(value.trim());
		}
	}
	return "Agent returned a response in an unrecognized format.";
}
...
export function AutoDraftStudioApp() {
	...
	const handleComparePanelReset = useCallback(() => {
		/* ComparePanel manages its own state; boundary reset is sufficient. */
	}, []);
  ...
  └── Review Queue       -> approve -> execute via .NET API`}
			...
			{activeTab === "compare" && (
				<AgentPanelBoundary onResetPanelCache={handleComparePanelReset}>
