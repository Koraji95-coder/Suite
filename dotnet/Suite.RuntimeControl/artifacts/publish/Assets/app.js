...
const WORKSPACE_META = {
  ...
  runtime: {
    ...
    views: [
      ...
      { id: "support", label: "Support", title: "Companions, logs, support bundle, and workstation controls." },
    ],
  },
};
...
function applyUtilityPaneWidth() {
  const width = Math.max(320, Math.min(640, Number(state.utilityPaneWidth) || 380));
...
function getOfficeLibraryState() {
  ...
  return {
    summary: firstDefined(library.summary, library.status, ""),
    totalDocuments: Number(firstDefined(library.totalDocumentCount, library.total, office.knowledge?.totalDocumentCount, 0)) || 0,
    documents,
    roots,
...
function buildCommandActions() {
  ...
  return [
    ...
    { kind: "companion", action: "launch", label: "Launch Office", description: "Open the Office companion app." },
...
function renderWorkspaceNav() {
  ...
  const pinnedActions = state.activeWorkspace === "office"
    ? [
        ...
        { kind: "companion", action: "launch", label: "Launch Office" },
      ...
      <section class="hero-shell office-hero">
        <div class="hero-copy">
          ...
          <div class="hero-actions">
            ${renderActionButton("Launch Office", { kind: "companion", action: "launch", label: "Launch Office" }, "action-btn primary")}
  ...
  if (view === "runtime") {
    dom.workspaceContent.innerHTML = `
      <section class="content-grid runtime-summary-grid">
        ...
        ${renderMetricPanel("Companion", companionStatus.label, "Office remains a managed companion after runtime trust is established.")}
      </section>
  ...
  dom.workspaceContent.innerHTML = `
    <section class="content-grid support-grid">
      ...
      <article class="content-panel quiet">
        <div class="lane-header compact">
          <div>
            <div class="panel-eyebrow">Companion</div>
            <h2>${escapeHtml(companionStatus.label)}</h2>
          </div>
        </div>
        <p>Office is managed as a companion app so it starts after Suite runtime readiness, not before it.</p>
        <div class="summary-pill-row">
    ...
    <section class="content-grid two-up">
      ...
      <article class="content-panel quiet">
        <div class="lane-header compact">
          <div>
            <div class="panel-eyebrow">Tooling audit</div>
            <h2>Codex local leverage</h2>
            <p>Current MCP servers stay visible here, and the next useful local skills are listed without forcing a new rollout.</p>
          </div>
        </div>
        <div class="signal-columns">
          <div class="signal-column">
            <div class="panel-eyebrow">Active MCP</div>
            <ul class="signal-list compact-list">${(tooling.activeMcpServers || []).map((item) => `<li>${escapeHtml(item)}</li>`).join("") || "<li>No active MCP servers were reported.</li>"}</ul>
          </div>
          <div class="signal-column">
            <div class="panel-eyebrow">Useful next skills</div>
            <ul class="signal-list compact-list">${(tooling.recommendedSkills || []).map((item) => `<li>${escapeHtml(item)}</li>`).join("") || "<li>No recommended skills were reported.</li>"}</ul>
          </div>
        </div>
      </article>
    </section>`;
...
function buildContextUtilityHtml() {
  ...
  if (state.activeWorkspace === "office") {
    return `
      ...
      <article class="context-card tone-${escapeHtml(companionStatus.tone)}">
        <div class="panel-eyebrow">Companion</div>
        <strong>${escapeHtml(companionStatus.label)}</strong>
        <p>Office launches through Runtime after local trust is established.</p>
      </article>
      </section>`;
  }
  ...
  return `
    ...
    <article class="context-card tone-${escapeHtml(companionStatus.tone)}">
      <div class="panel-eyebrow">Office companion</div>
      <strong>${escapeHtml(companionStatus.label)}</strong>
      <p>Runtime keeps Office as a managed companion instead of a separate startup surface.</p>
    </article>
