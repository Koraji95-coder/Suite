...
export function WhiteboardLibrary({ filterByPanel }: WhiteboardLibraryProps) {
  ...
  const [selectedPanel, setSelectedPanel] = useState<string>(filterByPanel || 'all');
  const [selectedTag, setSelectedTag] = useState<string>('all');
  const [viewingWhiteboard, setViewingWhiteboard] = useState<SavedWhiteboard | null>(null);
  ...
  const allPanels = ['all', ...Array.from(new Set(whiteboards.map(w => w.panel_context)))];
  const allTags = ['all', ...Array.from(new Set(whiteboards.flatMap(w => w.tags)))];

  const filteredWhiteboards = whiteboards.filter(wb => {
    const matchesSearch = wb.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      wb.tags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase()));
    const matchesPanel = selectedPanel === 'all' || wb.panel_context === selectedPanel;
    const matchesTag = selectedTag === 'all' || wb.tags.includes(selectedTag);
    return matchesSearch && matchesPanel && matchesTag;
  });

  if (loading) {
    return <div className="text-center text-orange-300 py-8">Loading whiteboards...</div>;
  }
