				...
				<div>
					...
					<div className="space-y-2">
						{components.map((c) => (
							<div
								key={c}
								className="bg-black/40 border border-orange-500/20 rounded p-2"
							>
								<div className="text-sm text-white/80 font-mono">{c}</div>
								<div className="text-xs text-orange-400/60">
									{COMPONENT_LINES[c]?.toLocaleString() || 0} lines
								</div>
							</div>
						))}
					</div>
