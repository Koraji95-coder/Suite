...
export function PanelInfoDialog({ title, sections, colorScheme: _colorScheme = 'cyan' }: PanelInfoDialogProps) {
  const [isOpen, setIsOpen] = useState(false);

  // Unified glass/orange style for all color schemes
  const colors = {
    button: 'bg-orange-500/20 hover:bg-orange-500/30 border-orange-500/40 text-white/90',
    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className={`bg-[#0a0a0a] backdrop-blur-xl border rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col ${colors.panel}`}>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {sections.map((section, index) => (
                <div key={index} className={`bg-white/[0.03] backdrop-blur-xl border rounded-lg p-6 ${colors.section}`}>
                  <h4 className={`text-xl font-bold mb-4 ${colors.text}`}>{section.title}</h4>
