...
import { type CalendarEvent, EventCalendar } from "../calendarindex";
...
export default function CalendarPage({
	...
	const mapRowToEvent = useCallback((row: CalendarEventRow): CalendarEvent => {
		...
		return {
			...
			color: (row.color as import("./calendartypes").EventColor) ?? undefined,
			source: "manual",
	...
	const handleEventAdd = async (event: CalendarEvent) => {
		try {
			...
			logger.error("CalendarPage", "Failed to add calendar event", {
				event,
				error: err,
			});
		}
	...
	const handleEventUpdate = async (updated: CalendarEvent) => {
		try {
			...
			logger.error("CalendarPage", "Failed to update calendar event", {
				event: updated,
				error: err,
			});
		}
	...
	const handleEventDelete = async (eventId: string) => {
		try {
			...
			logger.error("CalendarPage", "Failed to delete calendar event", {
				eventId,
				error: err,
			});
		}
