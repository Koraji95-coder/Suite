...
import { Badge, Button, Panel, Stack, Text } from "@/components/system/base";
import type {
...
function conductorRouteRefKey(
  ...
  return `${cableType}:${String(wireFunction || "")
    .trim()
    .toUpperCase()}`;
}
...
function buildJumperCadPath(
  ...
  if (Math.abs(dy) >= Math.abs(dx)) {
    ...
    const lift = Math.min(maxLift, clamp(verticalDelta * 0.16, 2.5, 9));
  ...
  const run = Math.min(
    Math.max(3, horizontalDelta * 0.3 - 1),
  ...
  for (const panel of Object.values(scanData.panels)) {
    for (const sideData of Object.values(panel.sides)) {
      for (const strip of sideData.strips) {
        ...
        const labels = Array.from({ length: terminalCount }, (_, index) =>
          String(index + 1),
        );
        strips.push({
...
interface ConduitTerminalWorkflowProps {
  onAutomationSnapshotChange?: (snapshot: AutoWireAutomationSnapshot) => void;
}
...
export function ConduitTerminalWorkflow({
  ...
  const automationQueueItems = useMemo<AutomationQueueItem[]>(() => {
    ...
    if (routeStats.pending > 0) {
      items.push({
        ...
        detail:
          "Pending routes should be included in the next schedule refresh.",
  ...
  useEffect(() => {
    onAutomationSnapshotChange?.({
      ...
      readyForPlan: Boolean(
        scanData && (routeStats.total > 0 || routeStats.pending > 0),
      ),
      summary: scanData
  ...
  const handleTerminalClick = (terminal: TerminalNode) => {
    ...
    const cadPath = isJumper
      ...
      : (() => {
          const fromEdge = terminalStripEdgePoint(
            from,
            layout.strips,
            terminal,
          );
          const toEdge = terminalStripEdgePoint(terminal, layout.strips, from);
          const fromLead = terminalLeadFromEdge(
            fromEdge,
            from.side,
            toEdge,
            36,
          );
          const toLead = terminalLeadFromEdge(
            toEdge,
            terminal.side,
            fromEdge,
            36,
        ...
        <Panel variant="glass" padding="md" className={styles.controlsCard}>
          <Stack gap={3}>
            ...
            <div className={styles.controlGroup}>
              ...
              >
                <span>
                  Override reason (required only when fail findings appear)
                </span>
                <textarea
          ...
          {selectedRoute ? (
            <div className={styles.selectedRoute}>
              ...
              <div>
                <span>Backcheck</span>
                <strong>
                  {backcheckStatusLabel(selectedRoute.cadBackcheckStatus)}
                </strong>
              </div>
