...
describe("supabase-local-snapshot helpers", () => {
	...
	it("builds stable snapshot ids with optional workstation and label tokens", () => {
		...
		expect(snapshotId).toBe("20260402-123456z-dev-work-recovery-lane");
	});

	it("lists and resolves snapshots by latest or explicit id", () => {
		const snapshotRoot = createTempDir("suite-snapshot-root-");
		const firstDir = path.join(snapshotRoot, "20260401-010101z-dev-work");
		const secondDir = path.join(snapshotRoot, "20260402-020202z-dev-work");
		fs.mkdirSync(firstDir, { recursive: true });
		fs.mkdirSync(secondDir, { recursive: true });
		fs.writeFileSync(
			path.join(firstDir, "manifest.json"),
			JSON.stringify({
				snapshotId: "20260401-010101z-dev-work",
		...
		fs.writeFileSync(
			...
			JSON.stringify({
				snapshotId: "20260402-020202z-dev-work",
		...
		expect(snapshots.map((snapshot) => snapshot.snapshotId)).toEqual([
			"20260402-020202z-dev-work",
			"20260401-010101z-dev-work",
		]);
		expect(resolveSnapshotSelection("latest", snapshotRoot)?.snapshotId).toBe(
			"20260402-020202z-dev-work",
		);
		expect(
			resolveSnapshotSelection("20260401-010101z-dev-work", snapshotRoot)
				?.snapshotId,
		).toBe("20260401-010101z-dev-work");
	});
});
