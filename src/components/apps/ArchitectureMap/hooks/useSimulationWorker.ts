...
export function useSimulationWorker(
			...
			const workerLinks: WorkerLink[] = s.links.map((l) => ({
				source:
					typeof l.source === "object"
						? (l.source as ArchNode).id
						: String(l.source),
				target:
					typeof l.target === "object"
						? (l.target as ArchNode).id
						: String(l.target),
				type: l.type,
			}));
	...
	const pinNode = useCallback(
		(idx: number, x: number | null, y: number | null) => {
			post({ type: "pin", nodeIndex: idx, fx: x, fy: y });
		},
		[post],
	);

	const setAlphaTarget = useCallback(
		(value: number) => {
	...
	const destroy = useCallback(() => {
		...
		if (w) {
			try {
				w.postMessage({ type: "stop" } satisfies ToWorkerMessage);
			} catch {
				/* already dead */
			}
			w.terminate();
