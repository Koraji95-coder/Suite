...
export function CalendarWidget({
  ...
  const navigateMonth = (direction: number) => {
    ...
    if (selectedDate) {
      ...
      onDateSelect(formatDateString(year, month + direction, Math.min(selectedDay, daysInNew)));
    }
  };
  ...
  const handleToday = () => {
    ...
    if (selectedDate) {
      const selectedDay = parseInt(selectedDate.split('-')[2]);
      const daysInNew = new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate();
      onDateSelect(formatDateString(now.getFullYear(), now.getMonth(), Math.min(selectedDay, daysInNew)));
    }
  };
  ...
  const renderDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      let urgencyStyleObj: { backgroundColor: string; color: string; borderColor: string; fontWeight: string } | null = null;
      if (hasDueDate) {
        ...
        const urgencyOrder: Record<string, number> = { OVERDUE: 0, CRITICAL: 1, WARNING: 2, NORMAL: 3 };
        ...
        urgencyStyleObj = getCalendarDayUrgencyStyle(allDatesOnDay.find(d => getUrgencyLevel(d) === bestLevel) || allDatesOnDay[0]);
      }

      const baseClass = 'h-10 flex items-center justify-center rounded-lg transition-all';
  ...
  const selectedDateContent = selectedDate && (() => {
    ...
    const projectsOnDate = projects.filter(p => p.deadline && p.deadline.split('T')[0] === selectedDate);
    ...
    return (
      ...
      >
        ...
        {tasksOnDate.map(task => {
          const project = projects.find(p => p.id === task.project_id) || allProjectsMap.get(task.project_id);
          return (
            <div
              key={task.id}
