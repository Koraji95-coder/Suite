import type { Point2D, Line2D, Direction, GridRod, GridConductor, GridPlacement, GridConfig } from './types';
...
export function parseRodsText(txt: string): GridRod[] {
  ...
  for (const raw of txt.split('\n')) {
    ...
    rods.push({ label, grid_x: x, grid_y: y, depth, diameter, sort_order: idx++ });
  }
  return rods;
}
...
export function parseConductorsText(txt: string): GridConductor[] {
  ...
  for (const raw of txt.split('\n')) {
    ...
    conductors.push({ label, length: isNaN(length) ? null : length, x1, y1, x2, y2, diameter, sort_order: idx++ });
  }
  return conductors;
}
...
export function findIntersectionPoints(lines: Line2D[]): Map<string, Point2D> {
  ...
  for (const h of hs) {
    ...
    for (const v of vs) {
      ...
      const x = vx1;
      if (x >= hx1 - EPS_MATCH && x <= hx2 + EPS_MATCH && y >= vy1 - EPS_MATCH && y <= vy2 + EPS_MATCH) {
        const p: Point2D = [x, y];
        pts.set(qpt(p), p);
      }
    }
...
export function splitLinesAtPoints(lines: Line2D[], pts: Map<string, Point2D>): Line2D[] {
  const atomic: Line2D[] = [];
  ...
  for (const ln of lines) {
    ...
    if (isH(ln)) {
      ...
      for (const [px, py] of allPts) {
        if (Math.abs(py - y) <= EPS_MATCH && px >= bx1 - EPS_MATCH && px <= bx2 + EPS_MATCH) {
          xs.push(px);
        }
      }
      xs.sort((a, b) => a - b);
      ...
      for (let i = 1; i < unique.length; i++) {
        if (Math.abs(unique[i] - unique[i - 1]) > EPS_MATCH) {
          atomic.push([[unique[i - 1], y], [unique[i], y]]);
        }
      }
    } else if (isV(ln)) {
      const x = ln[0][0];
      const ys: number[] = [];
      for (const [px, py] of allPts) {
        if (Math.abs(px - x) <= EPS_MATCH && py >= by1 - EPS_MATCH && py <= by2 + EPS_MATCH) {
          ys.push(py);
        }
      }
      ys.sort((a, b) => a - b);
      ...
      for (let i = 1; i < unique.length; i++) {
        if (Math.abs(unique[i] - unique[i - 1]) > EPS_MATCH) {
          atomic.push([[x, unique[i - 1]], [x, unique[i]]]);
        }
      }
    }
  }
...
export function buildDirectionMap(segments: Line2D[]): Map<string, Set<Direction>> {
...
export function classifyNodes(dirMap: Map<string, Set<Direction>>): { tees: Point2D[]; crosses: Point2D[] } {
  const tees: Point2D[] = [];
...
export function gridToAutocad(gx: number, gy: number, config: GridConfig): Point2D {
  const originXUnits = config.origin_x_feet * INCHES_PER_FOOT + config.origin_x_inches;
  const originYUnits = config.origin_y_feet * INCHES_PER_FOOT + config.origin_y_inches;
...
export function computeGridMaxY(rods: GridRod[], conductors: GridConductor[]): number {
  let maxY = 0;
...
export function generatePlacements(
  ...
  config: GridConfig,
): { placements: GridPlacement[]; segmentCount: number; teeCount: number; crossCount: number } {
  const gridLines: Line2D[] = conductors.map(c => [[c.x1, c.y1], [c.x2, c.y2]]);
  ...
  if (config.place_tees) {
    for (const [gx, gy] of tees) {
      ...
      placements.push({ type: 'TEE', grid_x: gx, grid_y: gy, autocad_x: ax, autocad_y: ay, rotation_deg: rot });
    }
  }
  ...
  return { placements, segmentCount: segments.length, teeCount: tees.length, crossCount: crosses.length };
}

export function conductorsToLines(conductors: GridConductor[]): Line2D[] {
  return conductors.map(c => [[c.x1, c.y1], [c.x2, c.y2]]);
}
