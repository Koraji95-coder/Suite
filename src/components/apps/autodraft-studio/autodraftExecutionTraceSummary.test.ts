	...
	return {
		...
		meta: {
			...
			executionReceipt: {
				...
				textSwapUpdates: [{ targetEntityId: "12" }, { targetEntityId: "34" }],
...
describe("autodraftExecutionTraceSummary", () => {
	it("summarizes receipt counts and cad context", () => {
		...
		expect(summary?.counts.textSwapUpdates).toBe(2);
	...
	it("builds revision notes with receipt and workflow details", () => {
		...
		expect(notes).toContain("Text swaps: 2");
	...
	it("builds a compact issue summary from receipt counts", () => {
		...
		expect(summary).toContain("2 text swap update(s)");
