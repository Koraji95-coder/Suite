...
def _metadata_from_tracker_state(state: Mapping[str, Any] | None) -> Dict[str, Any]:
    ...
    return {
        ...
        "isCreating": bool(state.get("isCreating")),
        "operationType": _optional_text(state.get("operationType")),
        "operationRequestId": _optional_text(state.get("operationRequestId")),
        "operationTargetPath": _optional_text(state.get("operationTargetPath")),
        "operationStartedAt": _timestamp_ms_from_value(state.get("operationStartedAt")) or 0,
...
class AutoCadStateCollector:
    ...
    def heartbeat(self, *, status: str = "online") -> Dict[str, Any]:
        ...
        metadata = {
            ...
            "isCreating": bool(snapshot.get("isCreating")),
            "operationType": _optional_text(snapshot.get("operationType")),
            "operationRequestId": _optional_text(snapshot.get("operationRequestId")),
            "operationTargetPath": _optional_text(snapshot.get("operationTargetPath")),
            "operationStartedAt": int(snapshot.get("operationStartedAt") or 0),
