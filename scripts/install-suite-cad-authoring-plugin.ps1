...
if (-not $ProjectPath) {
    $ProjectPath = Join-Path $repoRoot "dotnet\suite-cad-authoring\SuiteCadAuthoring.csproj"
}
$ProjectPath = (Resolve-Path $ProjectPath).Path
$projectDir = Split-Path -Parent $ProjectPath
$packageProductCode = "{0A2D1487-1B44-46D9-A507-17B419D679E3}"
$packageUpgradeCode = "{7B1C557C-85F5-4F6A-B815-8D0E2E3245CF}"
...
function Get-DefaultBundleRoot {
    ...
    return Join-Path $env:USERPROFILE "AppData\Roaming\Autodesk\ApplicationPlugins\SuiteCadAuthoring.bundle"
}
...
function Invoke-DotNetBuildWithRetry {
    ...
    while ($attempts -lt 3) {
        ...
        if ($attempts -ge 3) {
            throw "dotnet build failed for SuiteCadAuthoring."
        }
...
function Invoke-DotNetPublishWithRetry {
    ...
    while ($attempts -lt 3) {
        ...
        if ($attempts -ge 3) {
            throw "dotnet publish failed for SuiteCadAuthoring."
        }
        Start-Sleep -Seconds 3
    }
}
...
$bundleDllPath = Join-Path $bundleContentsDir "SuiteCadAuthoring.dll"
$bundleDepsPath = Join-Path $bundleContentsDir "SuiteCadAuthoring.deps.json"
$bundleRuntimeConfigPath = Join-Path $bundleContentsDir "SuiteCadAuthoring.runtimeconfig.json"
$bundlePdbPath = Join-Path $bundleContentsDir "SuiteCadAuthoring.pdb"
...
$builtDllPath = Join-Path $resolvedOutputDir "SuiteCadAuthoring.dll"
...
foreach ($outputFile in $outputFiles) {
    ...
    try {
        ...
        throw "CAD authoring plugin bundle file is locked at '$destinationPath'. Close AutoCAD, then rerun scripts/install-suite-cad-authoring-plugin.ps1.$processHint"
    }
    try {
