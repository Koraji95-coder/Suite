...
import { type ColorScheme, useTheme } from "@/lib/palette";
...
function computeBounds(nodes: ArchNode[]): Bounds {
	...
	if (nodes.length === 0)
		return { box, center: new THREE.Vector3(0, 0, 0), radius: 200 };
...
function Scene({
	...
	const coreMat = useMemo(() => {
		...
		try {
			return createHyphaeCoreNodeMaterial({
			...
			logger.warn(
				"ArchitectureMap",
				"Core node material failed (TSL/WebGPU), using fallback",
				{ error: e },
			);
			return null;
		}
	...
	useEffect(() => {
		return () => {
			if (
				coreMat &&
				"dispose" in coreMat &&
				typeof coreMat.dispose === "function"
			) {
				coreMat.dispose();
			}
		};
	...
	const curves = useMemo(
		() => buildCurves(graph.nodes, graph.links, palette),
		[graph.nodes, graph.links, palette],
	);
...
export function ArchitectureMap3D() {
	...
	const bounds = useMemo(
		() => (frozen ? computeBounds(frozen.nodes) : null),
		[frozen],
	);

	return (
		<div
			className="flex overflow-hidden bg-black/90 relative"
		...
		>
			...
			<div className="flex-1 relative">
				<Canvas
					...
					gl={async (p) => {
						try {
							...
							logger.warn(
								"ArchitectureMap",
								"WebGPU renderer init failed, using WebGL fallback",
								{ error: e },
							);
				...
				>
					...
					<div>
						<h2 className="text-xl font-bold text-white/80/80">
							Architecture Map (3D)
						</h2>
						<p className="text-orange-400/40 text-xs">
							orbit to explore - click nodes to inspect - scroll to zoom
						</p>
					</div>
				</div>
					...
					<div className="h-full flex flex-col">
						<div className="px-5 py-4 border-b border-orange-500/15">
							...
							<div className="text-xs text-orange-400/50">
								Click a node in the 3D map to see details.
							</div>
						</div>
