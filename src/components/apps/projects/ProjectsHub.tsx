import { AlertCircle, FolderCheck, FolderOpen } from "lucide-react";
import { useEffect, useState } from "react";
import { supabase } from "../../lib/supabase";
import { PROJECT_CATEGORIES, Project } from "./projectmanagertypes";
import {
	categoryColor,
	formatDateOnly,
	getDeadlineStatus,
...
export function ProjectsHub({ onSelectProject }: ProjectsHubProps) {
	...
	const categorized = PROJECT_CATEGORIES.map((cat) => ({
		cat,
										...
										>
											<div className="flex items-start justify-between mb-3">
												<div className="flex items-center space-x-3">
													...
													/>
													<h4 className="text-white/90 font-semibold group-hover:text-orange-50">
														{project.name}
													</h4>
												</div>
											</div>
											...
											<div className="flex items-center justify-between">
												...
												{project.deadline && (
													<div
														className={`flex items-center space-x-1 text-sm ${deadlineStatus.color}`}
													>
						...
						<div>
							<div className="flex items-center space-x-2 px-2 py-3">
								...
								<span className="text-white/50 text-sm">
									...
									{uncategorized.length === 1 ? "project" : "projects"})
								</span>
							</div>
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
