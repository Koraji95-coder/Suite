...
export function GridPreview3D({ rods, conductors, placements }: GridPreview3DProps) {
  ...
  useEffect(() => {
    ...
    const testWellSet = new Set(
      placements.filter(p => p.type === 'GROUND_ROD_TEST_WELL').map(p => `${p.grid_x},${p.grid_y}`)
    );
    ...
    const teeMat = new THREE.MeshStandardMaterial({ color: BLUE, emissive: new THREE.Color(BLUE).multiplyScalar(0.2) });
    const crossMat = new THREE.MeshStandardMaterial({ color: CYAN, emissive: new THREE.Color(CYAN).multiplyScalar(0.2) });
    ...
    for (const p of placements) {
      ...
      const marker = new THREE.Mesh(new THREE.BoxGeometry(0.012, 0.004, 0.012), mat);
      marker.position.set(x, 0.007, z);
      if (p.type === 'TEE') marker.rotation.y = (p.rotation_deg * Math.PI) / 180;
