...
export function SymmetricalComponents() {
  ...
  const complexMult = (c1: { x: number; y: number }, c2: { x: number; y: number }) => ({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <h2 className="text-3xl font-bold text-white/90">Symmetrical Components</h2>
      </div>

      <FrameSection title="Theory">
        <div className="space-y-3 text-white/80">
          <p>
            Any unbalanced three-phase system can be resolved into three balanced systems of phasors called
            <strong className="text-white/90"> symmetrical components</strong>:
          </p>
          <ul className="list-disc list-inside space-y-2 pl-4">
            <li>
              <strong className="text-white/90">Positive-sequence (1):</strong> Three phasors equal in magnitude,
              displaced 120° from each other, with same phase sequence as original (abc)
            </li>
            <li>
              <strong className="text-white/90">Negative-sequence (2):</strong> Three phasors equal in magnitude,
              displaced 120° from each other, with opposite phase sequence (acb)
            </li>
            <li>
              <strong className="text-white/90">Zero-sequence (0):</strong> Three phasors equal in magnitude and phase
      ...
      <FrameSection title="Symmetrical Components Calculator">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          <div className="space-y-3">
            <h4 className="text-lg font-semibold text-white/60">Phase A Voltage</h4>
            <div>
              <label className="block text-white/60 text-sm mb-1">Magnitude (V)</label>
              <input
                type="number"
                step="0.1"
                value={va.mag}
                onChange={(e) => setVa({ ...va, mag: parseFloat(e.target.value) || 0 })}
            ...
            <div>
              <label className="block text-white/60 text-sm mb-1">Angle (°)</label>
              <input
                type="number"
                step="0.1"
                value={va.angle}
                onChange={(e) => setVa({ ...va, angle: parseFloat(e.target.value) || 0 })}
                className="w-full bg-black/50 border border-orange-500/30 rounded-lg px-4 py-2 text-white/90 focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
          ...
          <div className="space-y-3">
            <h4 className="text-lg font-semibold text-white/60">Phase B Voltage</h4>
            <div>
              <label className="block text-white/60 text-sm mb-1">Magnitude (V)</label>
              <input
                type="number"
                step="0.1"
                value={vb.mag}
                onChange={(e) => setVb({ ...vb, mag: parseFloat(e.target.value) || 0 })}
                className="w-full bg-black/50 border border-orange-500/30 rounded-lg px-4 py-2 text-white/90 focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
            <div>
              ...
              <input
                ...
                step="0.1"
                value={vb.angle}
                onChange={(e) => setVb({ ...vb, angle: parseFloat(e.target.value) || 0 })}
                className="w-full bg-black/50 border border-orange-500/30 rounded-lg px-4 py-2 text-white/90 focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
          ...
          <div className="space-y-3">
            <h4 className="text-lg font-semibold text-white/60">Phase C Voltage</h4>
            <div>
              <label className="block text-white/60 text-sm mb-1">Magnitude (V)</label>
              <input
                type="number"
                step="0.1"
                value={vc.mag}
                onChange={(e) => setVc({ ...vc, mag: parseFloat(e.target.value) || 0 })}
                className="w-full bg-black/50 border border-orange-500/30 rounded-lg px-4 py-2 text-white/90 focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
            <div>
              ...
              <input
                ...
                step="0.1"
                value={vc.angle}
                onChange={(e) => setVc({ ...vc, angle: parseFloat(e.target.value) || 0 })}
                className="w-full bg-black/50 border border-orange-500/30 rounded-lg px-4 py-2 text-white/90 focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
        ...
        <div className="space-y-4">
          <h4 className="text-lg font-semibold text-white/60">Sequence Components</h4>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gradient-to-br from-gray-500/10 to-gray-600/10 border border-gray-500/30 rounded-lg p-4">
              <p className="text-gray-400 text-sm font-semibold mb-2">Zero-Sequence (V_a0)</p>
              <p className="text-white/90 font-mono text-xl mb-1">
            ...
            <div className="bg-gradient-to-br from-green-500/10 to-pink-600/10 border border-green-500/30 rounded-lg p-4">
              <p className="text-green-400 text-sm font-semibold mb-2">Positive-Sequence (V_a1)</p>
              <p className="text-white/90 font-mono text-xl mb-1">
            ...
            <div className="bg-gradient-to-br from-red-500/10 to-orange-600/10 border border-red-500/30 rounded-lg p-4">
              <p className="text-red-400 text-sm font-semibold mb-2">Negative-Sequence (V_a2)</p>
              <p className="text-white/90 font-mono text-xl mb-1">
        ...
        <div className="mt-6">
          ...
          {showWork && (
            <div className="mt-4 bg-gradient-to-br from-orange-500/10 to-amber-500/10 border border-orange-500/30 rounded-lg p-5">
              <h4 className="text-white/60 font-semibold mb-3">Step-by-Step Analysis</h4>
      ...
      <FrameSection title="Matrix Transformation">
        ...
        <div className="space-y-4">
          <div className="bg-black/50 border border-white/10 rounded-lg p-4">
            <h4 className="text-white/60 font-semibold mb-3">Phase to Sequence (Analytical Method)</h4>
          ...
          <div className="bg-black/50 border border-white/10 rounded-lg p-4">
            <h4 className="text-white/60 font-semibold mb-3">Sequence to Phase (Inverse Transformation)</h4>
            <div className="overflow-x-auto">
              <div className="font-mono text-white/90 text-sm space-y-2">
                <p>[V_a]   [1  1   1 ] [V_a0]</p>
                <p>[V_b] = [1  a²  a ] [V_a1]</p>
          ...
          <div className="bg-black/50 border border-white/10 rounded-lg p-4">
            <h4 className="text-white/60 font-semibold mb-3">Operator Properties</h4>
      ...
      <FrameSection title="Sequence Components for Other Phases">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-black/50 border border-white/10 rounded-lg p-4">
            <h4 className="text-white/60 font-semibold mb-2">Phase B Components</h4>
          ...
          <div className="bg-black/50 border border-white/10 rounded-lg p-4">
            <h4 className="text-white/60 font-semibold mb-2">Phase C Components</h4>
            <div className="space-y-1 text-white/90 font-mono text-sm">
              <p>V_c0 = V_a0</p>
              <p>V_c1 = a × V_a1</p>
              <p>V_c2 = a² × V_a2</p>
        ...
        <div className="mt-4 bg-gradient-to-br from-orange-500/10 to-amber-500/10 border border-orange-500/30 rounded-lg p-4">
          <p className="text-white/80 text-sm">
            <strong className="text-white/90">Note:</strong> All three phases have the same zero-sequence component.
            The positive and negative sequence components rotate according to the operator 'a'.
          </p>
        </div>
      </FrameSection>
      ...
      <FrameSection title="Example: Unbalanced System">
        <div className="space-y-2 text-white/80">
          <p>
            Given: V_a = 100∠0° V, V_b = 100∠-110° V, V_c = 100∠125° V (unbalanced angles)
          </p>
          <p className="font-semibold text-white/90 mt-3">Solution:</p>
          <p className="pl-4">• Zero-sequence: V_a0 = (1/3)(V_a + V_b + V_c) ≈ 3.92∠161.6° V</p>
          <p className="pl-4">• Positive-sequence: V_a1 = (1/3)(V_a + aV_b + a²V_c) ≈ 94.73∠-1.3° V</p>
          <p className="pl-4">• Negative-sequence: V_a2 = (1/3)(V_a + a²V_b + aV_c) ≈ 8.88∠-137.5° V</p>
          <p className="mt-3 text-white/60 text-sm">
            This shows the unbalanced system has both positive and negative sequence components, plus a small
            zero-sequence due to angle imbalance.
          </p>
        </div>
      </FrameSection>
    </div>
