...
import { isProjectStandardsLatestReviewBlocking } from "@/features/standards-checker/latestReview";
import type { StandardDocumentSourceMode } from "@/features/transmittal-builder";
...
import { buildNativeStandardsReviewDescriptor } from "@/features/project-review/descriptors";
import type {
...
export function buildProjectIssueSetEvidencePacket(args: {
	...
	const nativeStandardsReview = isProjectStandardsLatestReviewBlocking(
		args.nativeStandardsReview ?? null,
	)
		? args.nativeStandardsReview ?? null
		: null;
	const nativeStandardsDescriptor = nativeStandardsReview
		? buildNativeStandardsReviewDescriptor(nativeStandardsReview)
		: null;
	...
	const reviewDecisionItems = [
		...
		nativeStandardsDescriptor &&
		waivedStandardsIds.has(nativeStandardsDescriptor.itemId)
			? [
					{
						itemType: "standards" as const,
						status: "waived" as const,
						label: "Project standards review",
					},
				]
			: []),
	];
	...
	const waivedStandardsCount =
		standardsChecks.filter((row) => waivedStandardsIds.has(`standards:${row.id}`))
			.length +
		(nativeStandardsReview &&
		nativeStandardsDescriptor &&
		waivedStandardsIds.has(nativeStandardsDescriptor.itemId)
			? 1
			: 0);
	const nativeReviewMessages = nativeStandardsReview
		? [
				...nativeStandardsReview.warnings,
				...nativeStandardsReview.results
	...
	return {
		...
		standards: {
			...
			pendingCount: standardsChecks.filter((row) => row.qa_status === "pending")
				.length,
			nativeReview: {
				hasReview: Boolean(nativeStandardsReview),
				overallStatus: nativeStandardsReview?.overallStatus ?? null,
				recordedAt: nativeStandardsReview?.recordedAt ?? null,
				requestId: nativeStandardsReview?.requestId ?? null,
				standardsCategory: nativeStandardsReview?.standardsCategory ?? null,
				selectedStandardCount:
					nativeStandardsReview?.selectedStandardIds.length ?? 0,
				warningCount: nativeStandardsReview?.warnings.length ?? 0,
				inspectedDrawingCount: Number(
					nativeStandardsReview?.summary.inspectedDrawingCount ||
						nativeStandardsReview?.summary.drawingCount ||
						0,
				),
				providerPath:
					typeof nativeStandardsReview?.summary.providerPath === "string"
						? nativeStandardsReview.summary.providerPath
						: null,
				messages: nativeReviewMessages,
			},
...
export function renderProjectIssueSetEvidencePacketMarkdown(
	...
	for (const drawing of packet.titleBlock.drawings) {
		...
		lines.push(
			`- ${drawing.fileName} | ${drawing.drawingNumber || "No drawing number"} | ${detailParts.join(" â€¢ ")}`,
		);
		for (const issue of drawing.issues) {
			lines.push(`  - Issue: ${issue}`);
		}
	...
	if (packet.standards.nativeReview.hasReview) {
		lines.push(
			`- Native project review: ${packet.standards.nativeReview.overallStatus || "unknown"} | ${packet.standards.nativeReview.selectedStandardCount} standard(s) | ${packet.standards.nativeReview.inspectedDrawingCount} drawing(s) inspected | ${formatTimestamp(packet.standards.nativeReview.recordedAt)}`,
		);
		if (packet.standards.nativeReview.providerPath) {
			lines.push(
				`- Native provider: ${packet.standards.nativeReview.providerPath}`,
			);
		}
		for (const message of packet.standards.nativeReview.messages) {
			lines.push(`  - ${message}`);
		}
	} else {
		lines.push("- Native project review: Not recorded yet.");
	}
