// src/features/ground-grid-generation/ui/gridEngine.ts
import type {
	Direction,
	GridConductor,
	GridConfig,
...
// ✅ single source of truth for map-key precision
const KEY_DECIMALS = 6;
const KEY_SCALE = 10 ** KEY_DECIMALS;

function quant(v: number): number {
	return Math.round(v * KEY_SCALE);
}

function qpt(p: Point2D): string {
	return `${quant(p[0])},${quant(p[1])}`;
}
...
export function parseRodsText(txt: string): GridRod[] {
	...
	for (const raw of txt.split("\n")) {
		...
		const label = parts[0];
		const x = Number.parseFloat(parts[parts.length - 2]);
		const y = Number.parseFloat(parts[parts.length - 1]);
		const depth = parts.length >= 7 ? Number.parseFloat(parts[1]) : 20;
		const diameter = parts.length >= 7 ? Number.parseFloat(parts[4]) : 1.5;
...
export function parseConductorsText(txt: string): GridConductor[] {
	...
	for (const raw of txt.split("\n")) {
		...
		const label = parts[1];
		const length = Number.parseFloat(parts[2]);
		const x1 = Number.parseFloat(parts[3]);
		const y1 = Number.parseFloat(parts[4]);
		const x2 = Number.parseFloat(parts[6]);
		const y2 = Number.parseFloat(parts[7]);
		const diameter = Number.parseFloat(parts[5]) || 1.5;
...
export function classifyNodes(dirMap: Map<string, Set<Direction>>): {
	...
	for (const [k, ds] of dirMap) {
		...
		const pt: Point2D = [parts[0] / KEY_SCALE, parts[1] / KEY_SCALE];
...
export function generatePlacements(
	...
	// ✅ counts reflect actual placed items (after rod collisions skipped)
	let placedTees = 0;
	let placedCrosses = 0;
