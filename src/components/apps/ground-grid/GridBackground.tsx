...
function generateRandomGrid(): GridLayout {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      if (Math.random() > 0.3) {
        rods.push({ x: c * spacingX - width / 2, y: r * spacingY - height / 2 });
      }
    }
  }
  ...
  for (let r = 0; r < rows; r++) {
    ...
    if (skipGap) {
      ...
      hLines.push({ y: r * spacingY - height / 2, x1: -width / 2, x2: width / 2 });
    }
  }

  for (let c = 0; c < cols; c++) {
    vLines.push({ x: c * spacingX - width / 2, y1: -height / 2, y2: height / 2 });
  }
    ...
    const tick = () => {
      ...
      if (state.phase === 'fading') {
        ...
        state.gridGroup.traverse((obj) => {
          if (obj instanceof THREE.Line || obj instanceof THREE.Mesh) {
            ...
            if (mat.transparent) mat.opacity = mat.userData.baseOpacity * (1 - fadeProgress);
          }
        });
      ...
      if (state.phase === 'building') {
        ...
        state.gridGroup.traverse((obj) => {
          if (obj instanceof THREE.Line || obj instanceof THREE.Mesh) {
            const mat = obj.material as THREE.Material & { opacity: number };
            if (mat.transparent) mat.opacity = (mat.userData.baseOpacity || 0.6) * buildProgress;
          }
        });
