...
Status: this is the canonical low-level CAD transport reference. Active project setup/title block flows now go through browser -> hosted core -> Runtime Control localhost actions first, and Runtime Control may dispatch into the named-pipe bridge or the in-process CAD host underneath.
