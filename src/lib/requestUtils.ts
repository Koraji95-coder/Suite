...
import { TimeoutError, NetworkError, type Result, ok, err } from './errorHandling';
...
export async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      logger.debug(`Fetch attempt ${attempt + 1}/${maxRetries + 1}`, 'fetchWithRetry', {
        url,
      });
  ...
  while (true) {
    ...
    try {
      ...
      if (condition ? condition(result) : result) {
        logger.debug(`Polling succeeded after ${attempt + 1} attempts`, 'pollUntilSuccess');
        return ok(result);
      }
    } catch (error) {
      logger.debug(`Polling attempt ${attempt + 1} failed`, 'pollUntilSuccess', {
