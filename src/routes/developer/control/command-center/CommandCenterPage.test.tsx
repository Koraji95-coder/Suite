...
describe("CommandCenterPage", () => {
	it("ignores unsupported tab query parameters and stays on the command center route", async () => {
		render(
			<MemoryRouter
				initialEntries={["/app/developer/control/command-center?tab=watchdog"]}
			>
				<Routes>
					<Route
		...
		expect(screen.queryByText("/app/home?focus=watchdog")).toBeNull();
	...
	it("keeps architecture routing out of the command center surface", async () => {
		render(
			<MemoryRouter
				initialEntries={[
					"/app/developer/control/command-center?tab=architecture",
				]}
			>
				<Routes>
					<Route
	...
	it("keeps diagnostics-focused frontend and backend probes on the developer commands tab", () => {
		render(
			<MemoryRouter initialEntries={["/app/developer/control/command-center"]}>
				<Routes>
					<Route
		...
		expect(screen.getByText("Frontend Health Probe")).toBeTruthy();
		expect(screen.getByText("curl -sS http://127.0.0.1:5173 | cat")).toBeTruthy();
