...
export interface UseProjectWatchdogTelemetryOptions {
	includeTrackedDrawings?: boolean;
...
export function useProjectWatchdogTelemetry(
	...
	options: UseProjectWatchdogTelemetryOptions = {},
): ProjectWatchdogTelemetry {
	const includeTrackedDrawings = options.includeTrackedDrawings ?? true;
		...
		const run = async () => {
			...
			const [
				...
				includeRecentEvents
					...
					: Promise.resolve({ events: [] as WatchdogCollectorEvent[] }),
			...
			if (includeTrackedDrawings) {
				try {
					await syncSharedDrawingActivityFromLocalRuntime();
				} catch (error) {
					logger.debug(
						"Shared drawing activity sync skipped; using stored project drawing segments only.",
			...
			if (includeOverview && overviewResult.status === "fulfilled") {
			...
			setRecentEvents(
				includeRecentEvents && eventsResult.status === "fulfilled"
			...
			const primaryFailure = [
				includeOverview ? overviewResult : null,
				includeRecentEvents ? eventsResult : null,
				sessionsResult,
				collectorsResult,
				ruleResult,
			].find((result) => result && result.status === "rejected");
			if (primaryFailure && primaryFailure.status === "rejected") {
				setError(
					primaryFailure.reason instanceof Error
						? primaryFailure.reason.message
						: "Project telemetry is unavailable.",
				);
			}
			setTrackedDrawings(
				includeTrackedDrawings
	...
	return {
		...
		onlineCollectorCount:
			overview?.collectors.online ??
			allCollectors.filter((collector) => collector.status === "online").length,
