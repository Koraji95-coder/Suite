...
import { useProjectWatchdogTelemetry } from "@/components/apps/projects/useProjectWatchdogTelemetry";
...
function buildWarningQueueItemId(label: string) {
	return `autowire:warning:${label.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
}
...
export function TerminalAuthoringPanel({
	...
	const [operations, setOperations] = useState<TerminalAuthoringPreviewOperation[]>(
		[],
	);
	...
	const scopeResolvable =
		...
		Boolean(studioContext.drawingRootPath || selectedDrawingPaths.every((path) => /^[a-z]:[\\/]/i.test(path)));
	...
	const trackedDrawingByKey = useMemo(() => {
		...
		for (const drawing of telemetry.trackedDrawings) {
			const key = normalizeDrawingKey(drawing.drawingPath || drawing.drawingName);
			if (!key) continue;
			map.set(key, {
	...
	const groupedDrawings = useMemo(() => {
		...
		for (const operation of operations) {
			...
			const drawingKey = normalizeDrawingKey(drawingPath || operation.drawingName);
	...
	}, [activeSessionByDrawingKey, operations, selectedOperationIdSet, trackedDrawingByKey]);
	...
	const queueItems = useMemo<AutomationQueueItem[]>(() => {
		...
		return items;
	}, [approvedItemIds, groupedDrawings, scheduleSnapshot?.workbookFileName, unresolvedCount]);
	...
	const handleImportSchedule = async (event: ChangeEvent<HTMLInputElement>) => {
		...
		try {
			...
			const errorMessage =
				error instanceof Error ? error.message : "Terminal schedule import failed.";
	...
	useEffect(() => {
		...
		onAutomationSnapshotChange({
			...
			drawingName:
				drawingSummaries.length === 1 ? drawingSummaries[0]?.drawingName ?? null : null,
			terminalCount:
			...
			stripUpdateCount:
				operations.filter((operation) => operation.operationType === "label-upsert")
					.length,
			routeUpsertCount:
				operations.filter(
					(operation) =>
	...
	const handlePreview = async () => {
		if (!scheduleSnapshot || !studioContext.projectId || !issueSetSelected || !scopeResolvable) {
			return;
		}
		...
		try {
			...
			setWarnings([
				...
				...result.warnings.filter((warning) => !scheduleSnapshot.warnings.includes(warning)),
			]);
			...
			<div className={styles.headerRow}>
				...
				<div className={styles.actionRow}>
					<Button
						...
						size="sm"
						iconLeft={
							importingSnapshot ? <LoaderCircle size={14} /> : <Upload size={14} />
						}
			...
			<section className={styles.section}>
				...
				<div className={styles.scheduleGrid}>
					<div className={styles.snapshotCard}>
						<strong>
							{scheduleSnapshot?.workbookFileName || "No terminal schedule loaded"}
						</strong>
				...
				{operations.length === 0 ? (
					...
					<div className={styles.previewGroups}>
						{groupedDrawings.map((drawing) => {
							...
							const approved = itemId ? approvedItemIds.includes(itemId) : false;
							return (
										...
										<div className={styles.previewGroupIdentity}>
											...
											<div>
												...
												<p className={styles.groupDetail}>
													{drawing.selectedOperationCount}/{drawing.operationCount} selected •{" "}
													{drawing.liveSessionStatus
												...
												>
													...
													{operation.before || operation.after ? (
														<div className={styles.previewDiff}>
															{operation.before ? (
																<div>
																	<span className={styles.diffDanger}>−</span>
																	{operation.before}
																</div>
															) : null}
															{operation.after ? (
			...
			{reportId ? (
				<div className={styles.auditBar}>
					...
					<span className={styles.previewMeta}>
						...
						{managedRouteUpsertCount === 1 ? "" : "s"} • {changedDrawingCount}{" "}
						drawing
