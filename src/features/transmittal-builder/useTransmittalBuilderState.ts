...
export function useTransmittalBuilderState(
	...
	const [packageRegisterSnapshotId, setPackageRegisterSnapshotId] = useState<
		string | null
	>(null);
	const [nativeStandardsReview, setNativeStandardsReview] =
		useState<TransmittalNativeStandardsReviewSnapshot | null>(null);
	const [nativeStandardsReviewLoading, setNativeStandardsReviewLoading] =
		useState(false);
	const [nativeStandardsReviewError, setNativeStandardsReviewError] = useState<
		string | null
	>(null);
	...
	useEffect(() => {
		if (
			...
			setNativeStandardsReview(null);
			setNativeStandardsReviewError(null);
			setNativeStandardsReviewLoading(false);
			return;
		}

		let active = true;
		setNativeStandardsReviewLoading(true);
		setNativeStandardsReviewError(null);

		const loadNativeStandardsReview = async () => {
			const result = await standardsCheckerBackendService.fetchLatestReview(
				draft.selectedProjectId,
			);
			if (!active) {
				return;
			...
			startTransition(() => {
				setNativeStandardsReview(
					buildTransmittalNativeStandardsReviewSnapshot(result.data),
				);
				setNativeStandardsReviewError(result.error?.message ?? null);
				setNativeStandardsReviewLoading(false);
			});
		};

		void loadNativeStandardsReview();
		return () => {
			active = false;
		};
	}, [draft.selectedProjectId, draft.transmittalType]);
