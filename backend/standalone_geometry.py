...
class StandaloneConfig:
    ...
    input_file: str  # Path to DXF/DWG file
    output_excel: str  # Path for output Excel file
    target_layer: str  # Layer name to search for geometry
    extraction_mode: str  # "center" or "corners"
    point_prefix: str = "P"
    start_number: int = 1
    decimal_places: int = 3
    use_blocks: bool = True  # Also search inside block definitions
    include_modelspace: bool = True  # Also search ModelSpace
    verbose: bool = False
...
class StandaloneRow:
    ...
    north: float
    elevation: float
    corner: Optional[str] = None  # "NW", "NE", "SW", "SE" or None
...
def bbox_corners(points: List[Point3D]) -> List[Tuple[Point3D, str]]:
    ...
    Using convention: max-Y = North, max-X = East
    """
    if not points:
        return []
...
def extract_entity_points(entity: Any) -> List[Point3D]:
    ...
    - INSERT (block reference): insertion point
    """
    if entity is None:
        return []
    ...
    try:
        if ent_type in ("LWPOLYLINE", "POLYLINE"):
            ...
            # Block reference - get insertion point
            ...
            # Generic fallback - try to get bounding box center
            try:
                bbox = entity.bbox
                if bbox:
                    minpt, maxpt = bbox
...
def load_cad_file(filepath: str) -> Optional[Any]:
    ...
    Returns the DWG document or None if file cannot be loaded.
    """
    if not ezdxf:
    ...
    try:
        # For DWG files, ezdxf provides limited support
        # For full support, would need LibreDWG converter
...
def extract_geometries_from_file(
    ...
    Parameters:
        config: StandaloneConfig instance
        progress_callback: Optional callback(current: int, total: int, message: str)
    
    Returns:
        List of StandaloneRow objects
    """
    ...
    # Get entities on the target layer in ModelSpace
...
def export_to_excel(config: StandaloneConfig, rows: List[StandaloneRow], output_path: Optional[str] = None) -> str:
    ...
    Parameters:
        config: StandaloneConfig instance
        rows: List of StandaloneRow objects
...
def process_file(
    ...
    Returns:
        Tuple of (extracted_rows, output_excel_path)
    """
    # Extract geometries
...
# -------------------------
def cli_main():
    """Command-line interface for standalone geometry processing."""
    import argparse
    import json
...
Examples:
  ...
  python standalone_geometry.py file.dwg -c config.json
    ...
    parser.add_argument("-o", "--output", help="Output Excel file (default: input_extracted.xlsx)")
    parser.add_argument("-m", "--mode", choices=["center", "corners"], default="center",
                       help="Extraction mode: center or corners (default: center)")
