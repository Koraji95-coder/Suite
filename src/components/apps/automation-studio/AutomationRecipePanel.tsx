...
} from "@/services/projectCadWritePassService";
import {
...
function summarizeMarkupSnapshots(snapshots: ProjectMarkupSnapshotRecord[]) {
	...
	const drawingCount = new Set(
		snapshots.map((snapshot) => normalizePathKey(snapshot.drawingPath)).filter(Boolean),
	).size;
...
export function AutomationRecipePanel({
	...
	const [recipeId, setRecipeId] = useState<string>(() => createLocalId("recipe"));
	...
	>([]);
	const [selectedMarkupSnapshotIds, setSelectedMarkupSnapshotIds] = useState<string[]>(
		[],
	);
	...
	useEffect(() => {
		setWorkPackageId(selectedIssueSet?.workPackageId || createLocalId("work-package"));
		setRecipeId(selectedIssueSet?.recipeSnapshotId || createLocalId("recipe"));
	}, [selectedIssueSet?.id, selectedIssueSet?.recipeSnapshotId, selectedIssueSet?.workPackageId]);
	...
	}, [autoDraftSnapshot?.queueItems.length, autoWireSnapshot?.scheduleSnapshotId, autoWireSnapshot?.stripRows, cadUtilitySnapshot?.rules]);
	...
	useEffect(() => {
		...
		void (async () => {
			...
			const result = await projectMarkupSnapshotService.fetchSnapshots(projectId);
			if (cancelled) {
				return;
			}
			if (result.error) {
	...
	const availableMarkupSnapshots = useMemo(() => {
		const selectedDrawingKeys = new Set(
			(studioContext.selectedDrawingPaths ?? []).map((value) => normalizePathKey(value)),
		);
	...
	useEffect(() => {
		setSelectedMarkupSnapshotIds((current) => {
			const availableIds = new Set(availableMarkupSnapshots.map((snapshot) => snapshot.id));
	...
	const draftWorkPackage = useMemo<ProjectAutomationWorkPackageRecord>(() => {
		...
		return {
			...
			titleBlockSnapshotStatus:
				warningCount > 0 ? "needs-review" : "ready",
			titleBlockWarningCount: warningCount,
			createdAt: new Date().toISOString(),
			updatedAt: new Date().toISOString(),
			warnings: [
				...(autoDraftSnapshot?.warnings ?? []),
				...selectedMarkupSnapshots.flatMap((snapshot) => snapshot.warnings ?? []),
	...
	const draftRecipe = useMemo<ProjectAutomationRecipeRecord>(() => {
		...
		const autoWireApproved = autoWireSnapshot?.selectedOperationIds?.length ?? 0;
		const cadUtilsPlanned = cadUtilitySnapshot?.matchCount ?? 0;
		const cadUtilsApproved = cadUtilitySnapshot?.selectedPreviewKeys?.length ?? 0;
		return {
			id: recipeId,
			projectId: studioContext.projectId || "",
			issueSetId: studioContext.issueSetId,
			...
			steps: [
				{
					...
					actionable: selectedMarkupSnapshots.length > 0 && autoDraftPlanned > 0,
					plannedItemCount: autoDraftPlanned,
					approvedItemCount: autoDraftApproved,
					warningCount: autoDraftWarnings,
					bindingKinds: ["title-block", "drawing-row", "deliverable-row", "note-only"],
					summary:
						summarizeMarkupSnapshots(selectedMarkupSnapshots),
	...
	const enabledStepCount = draftRecipe.steps.filter((step) => step.enabled).length;
	...
	const saveDraftPackage = async () => {
		...
		try {
			...
			const nextWorkPackageId = workPackageResult.data?.id ?? draftWorkPackage.id;
	...
	const runApply = async () => {
		try {
			...
			const runSaveResult = await projectAutomationRecipeService.saveRun(nextRun);
			...
			const receiptResult = await projectAutomationReceiptService.saveReceipt({
				...
				reviewItemCount: applyResult.operations.filter((entry) => entry.approved).length,
				...
				markupSnapshotIds: selectedMarkupSnapshots.map((snapshot) => snapshot.id),
				terminalScheduleSnapshotId: saved.workPackage.terminalScheduleSnapshotId,
	...
	const downloadPrimaryReport = async (reportId: string, filename?: string | null) => {
		try {
	...
	if (!selectedIssueSet) {
		return (
			<Panel variant="support" padding="lg">
				<div className={styles.emptyState}>
					Select an issue set before building a shared offline work package.
					The recipe runner binds previews, snapshots, reports, and verification
		...
		<Panel variant="support" padding="lg">
			<div className={styles.root}>
							...
							<label className={styles.toggleControl}>
								<input
									...
									onChange={(event) =>
										setEnabledSources((current) => ({
											...current,
											[step.source === "cad-utils"
												? "cadUtils"
												: step.source]: event.target.checked,
										}))
									}
								/>
				...
				<div className={styles.toggleCard}>
					<div className={styles.toggleHeader}>
						...
						<Badge
							variant="soft"
							color={
								selectedMarkupSnapshots.length > 0 ? "success" : "warning"
							}
						>
					...
					<p className={styles.metaText}>
						Use reviewed Bluebeam page bundles already published to the
						project. Only snapshots bound to the selected issue-set drawings are
						eligible here.
					</p>
					{loadingMarkupSnapshots ? (
						...
						<div className={styles.stepList}>
							{availableMarkupSnapshots.map((snapshot) => {
								const operationCount = readMarkupSnapshotOperations(snapshot).length;
								...
								return (
									<label key={snapshot.id} className={styles.toggleControl}>
										...
										/>
										<span>
											{snapshot.drawingName || snapshot.drawingPath} • page{" "}
											{snapshot.pageIndex + 1} • {operationCount} op
											{operationCount === 1 ? "" : "s"}
										</span>
									</label>
								);
				...
				<div className={styles.summaryGrid}>
					...
					<div className={styles.summaryCard}>
						<div className={styles.summaryHeader}>
							...
							>
								{preflightResult?.ok ? "ready" : preflightResult ? "review" : "idle"}
							</Badge>
						</div>
						<p className={styles.metaText}>
						...
						<div className={styles.metaList}>
							...
							<span className={styles.metaChip}>
								ACADE {acadeResult?.acadeProjectFilePath ? "linked" : "unchecked"}
							</span>
						</div>
					</div>
					<div className={styles.summaryCard}>
						<div className={styles.summaryHeader}>
							...
							<Badge variant="soft" color={previewResult ? "success" : "default"}>
				...
				{primaryArtifacts.length ? (
						...
						{primaryArtifacts.map((artifact) => (
							<div key={artifact.id} className={styles.artifactCard}>
								...
								<p className={styles.artifactText}>
									{artifact.description || artifact.path || "Automation artifact"}
								</p>
					...
					<div className={styles.emptyState}>
						No shared run artifacts yet. Save the package, run preflight, preview
						the recipe, then apply and verify to capture the offline bundle.
					</div>
				)}
			</div>
		</Panel>
