...
export function AgentPanel() {
	...
	const handlePair = async () => {
		...
		if (success) {
			setResult({
				success: true,
				data: { message: "Successfully paired with agent!" },
			});
		}
	};
	...
	return (
		<div className="p-6 space-y-6">
			<div className="bg-white rounded-lg shadow p-6">
				...
				<section className="mb-6">
					...
					<div className="grid grid-cols-2 gap-3">
						<button
							onClick={() =>
								runTask(() => agentService.researchStandard("IEEE 80"))
							}
						...
						<button
							onClick={() =>
								runTask(() => agentService.researchStandard("NEC Article 250"))
							}
							className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600"
							disabled={loading}
						>
				...
				<section className="mb-6">
					...
					<div className="grid grid-cols-2 gap-3">
						...
						<button
							onClick={() =>
								runTask(() =>
									agentService.recallSimilarProjects(
										"urban high voltage substation",
									),
								)
							}
							className="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600"
			...
			{result && !loading && (
				<div
					className={`rounded-lg p-6 ${result.success ? "bg-green-50" : "bg-red-50"}`}
				>
