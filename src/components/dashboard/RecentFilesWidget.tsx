...
export function RecentFilesWidget() {
  ...
  return (
    ...
    >
      ...
      {loading ? (
        <div style={{ fontSize: 12, color: palette.textMuted, padding: '12px 0' }}>
          Loading...
        </div>
      ) : files.length === 0 ? (
        <div style={{ fontSize: 12, color: palette.textMuted, padding: '12px 0' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {files.map(f => (
            ...
            >
              <FileText size={14} color={palette.textMuted} style={{ flexShrink: 0 }} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{
                  fontSize: 12, fontWeight: 500, color: palette.text,
              ...
              <span style={{ fontSize: 10, color: palette.textMuted, flexShrink: 0, whiteSpace: 'nowrap' }}>
                {formatTime(f.accessed_at)}
              </span>
              <ExternalLink size={10} color={palette.textMuted} style={{ flexShrink: 0, opacity: 0.4 }} />
            </button>
          ))}
        </div>
      )}
