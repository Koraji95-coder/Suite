...
import type { AutomationWorkbenchMode } from "@/components/apps/automation-studio/automationStudioModels";
...
export const projectAutomationReceiptService = {
	...
	async saveReceipt(input: ProjectAutomationReceiptInput): Promise<{
		...
		const record: ProjectAutomationReceiptRecord = {
			...
			issueTagUpsertCount: Math.max(
				0,
				Number(input.issueTagUpsertCount || 0),
			),
			titleBlockUpdateCount: Math.max(
				0,
		...
		const persistError = await persistReceipts(projectId, [
			record,
			...existing.data,
		]);
		return {
