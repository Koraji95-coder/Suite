...
import { CheckCircle, XCircle, AlertTriangle, Upload, FileText, Search, Download, Eye, Settings as SettingsIcon, Zap } from 'lucide-react';
...
interface QARule {
  ...
  name: string;
  description: string;
  category: 'title_block' | 'layer' | 'dimension' | 'text' | 'compliance' | 'standard';
...
export function QAQCChecker() {
  ...
  const [selectedDrawing, setSelectedDrawing] = useState<DrawingAnnotation | null>(null);
  ...
  const loadRules = async () => {
    const defaultRules: QARule[] = [
      ...
      },
      {
        ...
        description: 'Verify layers follow naming conventions (e.g., E-POWR, E-LTNG)',
        category: 'layer',
        severity: 'warning',
        enabled: true,
      },
  ...
  const checkDrawing = async (drawingName: string) => {
    ...
    setTimeout(async () => {
      ...
      const qa_status = issues.some(i => i.severity === 'error') ? 'fail' :
  ...
  const filteredDrawings = drawings.filter(drawing => {
    const matchesSearch = drawing.drawing_name.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus = filterStatus === 'all' || drawing.qa_status === filterStatus;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <div className="p-3 bg-gradient-to-br from-green-500/20 to-emerald-500/20 rounded-lg">
            <CheckCircle className="w-8 h-8 text-green-400 animate-pulse" style={{ animationDuration: '2s' }} />
          </div>
          <div>
            <h2 className="text-3xl font-bold text-green-200">QA/QC Standards Checker</h2>
            <p className="text-green-400/70">Automated drawing compliance verification</p>
          </div>
        </div>
        <div className="flex items-center space-x-3">
          <button
            onClick={() => setShowRulesModal(true)}
            className="flex items-center space-x-2 bg-black/30 border border-green-500/30 hover:border-green-500/50 text-green-300 px-6 py-3 rounded-lg transition-all"
          >
      ...
      <FrameSection>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="relative md:col-span-2">
            ...
            <input
              ...
              className="w-full pl-10 bg-black/50 border border-green-500/30 rounded-lg px-4 py-2 text-green-100 focus:outline-none focus:ring-2 focus:ring-green-500"
            />
          </div>
          ...
          <div>
            <select
              ...
              className="w-full bg-black/50 border border-green-500/30 rounded-lg px-4 py-2 text-green-100 focus:outline-none focus:ring-2 focus:ring-green-500"
            >
        ...
        <div className="flex items-center justify-between mt-4 text-sm">
          <div className="flex items-center space-x-4 text-green-300">
            ...
            <span>Pass: {drawings.filter(d => d.qa_status === 'pass').length}</span>
            <span>Warning: {drawings.filter(d => d.qa_status === 'warning').length}</span>
            <span>Fail: {drawings.filter(d => d.qa_status === 'fail').length}</span>
          </div>
      ...
      {loading ? (
        <div className="text-center text-green-300 py-12">Loading drawings...</div>
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <h3 className="text-lg font-bold text-green-100">{drawing.drawing_name}</h3>
                  </div>
                </div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex items-center justify-between text-green-300/70">
                    ...
                    <span className="font-semibold text-green-200">{drawing.issues_found}</span>
                  </div>

                  <div className="flex items-center justify-between text-green-300/70">
                    <span>Status:</span>
                    <span className={`capitalize font-semibold ${
                      drawing.qa_status === 'pass' ? 'text-green-400' :
                      drawing.qa_status === 'fail' ? 'text-red-400' :
                      drawing.qa_status === 'warning' ? 'text-yellow-400' :
                      'text-gray-400'
                    }`}>
                      {drawing.qa_status}
                    </span>
                  ...
                  {drawing.checked_at && (
                    <div className="flex items-center justify-between text-green-300/70">
                      <span>Checked:</span>
                      <span>{new Date(drawing.checked_at).toLocaleDateString()}</span>
                    </div>
                  )}
                </div>
      ...
      {showUploadModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0a0a0a] backdrop-blur-xl border border-green-500/30 rounded-lg p-6 max-w-md w-full">
            <h3 className="text-2xl font-bold text-green-200 mb-4">Check Drawing</h3>
            {checkingDrawing ? (
              <div className="text-center py-8">
                <Zap className="w-12 h-12 text-green-400 animate-pulse mx-auto mb-4" />
                <p className="text-green-300">Running QA/QC checks...</p>
                <p className="text-green-400/60 text-sm mt-2">Applying {rules.filter(r => r.enabled).length} rules</p>
              </div>
            ) : (
              <form onSubmit={handleUpload} className="space-y-4">
                <div>
                  <label className="block text-green-300 text-sm font-medium mb-2">Drawing Name *</label>
                  <input
                    type="text"
                ...
                <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3">
                  <p className="text-green-300 text-sm">
                    {rules.filter(r => r.enabled).length} QA/QC rules will be applied to this drawing.
                  </p>
                </div>
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button
                    ...
                    className="bg-black/50 border border-green-500/30 text-green-300 hover:bg-green-500/10 px-6 py-2 rounded-lg transition-all"
                  >
                    Cancel
                  </button>
                </div>
      ...
      {showRulesModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0a0a0a] backdrop-blur-xl border border-green-500/30 rounded-lg p-6 max-w-3xl w-full max-h-[80vh] overflow-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-bold text-green-200">QA/QC Rules Configuration</h3>
              <button
              ...
              {rules.map(rule => (
                <div
                  ...
                  className={`bg-black/40 border rounded-lg p-4 transition-all ${
                    rule.enabled ? 'border-green-500/30' : 'border-gray-500/30 opacity-60'
                  }`}
                >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        <input
                          ...
                          className="w-5 h-5 rounded border-green-500/30 bg-black/50"
                        />
                        <h4 className="text-lg font-semibold text-green-200">{rule.name}</h4>
                        <span className={`text-xs px-2 py-1 rounded-full border ${getSeverityColor(rule.severity)}`}>
                          {rule.severity}
                        </span>
                      </div>
                      <p className="text-green-300/70 text-sm ml-8">{rule.description}</p>
      ...
      {selectedDrawing && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0a0a0a] backdrop-blur-xl border border-green-500/30 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-auto">
            <div className="flex items-center justify-between p-6 border-b border-green-500/30 sticky top-0 bg-gray-900/95 backdrop-blur-sm z-10">
              <div className="flex items-center space-x-3">
                {getStatusIcon(selectedDrawing.qa_status)}
                <div>
                  <h3 className="text-2xl font-bold text-green-200">{selectedDrawing.drawing_name}</h3>
                  <p className="text-green-400/70 text-sm">
                    Checked on {selectedDrawing.checked_at ? new Date(selectedDrawing.checked_at).toLocaleString() : 'N/A'}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-black/40 border border-green-500/30 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-green-200">{selectedDrawing.issues_found}</div>
                  <div className="text-green-400/70 text-sm mt-1">Issues Found</div>
                </div>
                <div className="bg-black/40 border border-green-500/30 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-green-200 capitalize">{selectedDrawing.qa_status}</div>
                  <div className="text-green-400/70 text-sm mt-1">Status</div>
                </div>
                <div className="bg-black/40 border border-green-500/30 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-green-200">{selectedDrawing.rules_applied.length}</div>
                  <div className="text-green-400/70 text-sm mt-1">Rules Applied</div>
                </div>
              </div>

              <div>
                <h4 className="text-lg font-bold text-green-200 mb-3">Issues Detected</h4>
                {selectedDrawing.annotations.length === 0 ? (
                  <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4 text-center">
                    <CheckCircle className="w-12 h-12 text-green-400 mx-auto mb-2" />
                    <p className="text-green-300">No issues found! Drawing passes all checks.</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {selectedDrawing.annotations.map((issue: Issue, idx: number) => (
                      <div
                        key={idx}
                        className={`bg-black/40 border rounded-lg p-4 ${
                          issue.severity === 'error' ? 'border-red-500/40' :
                          issue.severity === 'warning' ? 'border-yellow-500/40' :
                          'border-blue-500/40'
                        }`}
                      >
                        <div className="flex items-start space-x-3">
                          {issue.severity === 'error' && <XCircle className="w-5 h-5 text-red-400 flex-shrink-0 mt-0.5" />}
                          {issue.severity === 'warning' && <AlertTriangle className="w-5 h-5 text-yellow-400 flex-shrink-0 mt-0.5" />}
                          {issue.severity === 'info' && <FileText className="w-5 h-5 text-blue-400 flex-shrink-0 mt-0.5" />}
                          <div className="flex-1">
                            <div className="flex items-center space-x-2 mb-1">
                              <span className={`text-xs px-2 py-1 rounded-full border capitalize ${getSeverityColor(issue.severity)}`}>
                                {issue.severity}
                              </span>
                              <span className="text-xs text-green-400/60 capitalize">{issue.type.replace('_', ' ')}</span>
                            </div>
                            <p className="text-green-100">{issue.message}</p>
                            {issue.location && (
                              <p className="text-green-400/60 text-sm mt-1">Location: {issue.location}</p>
                            )}
                          </div>
                        </div>
                      </div>
