...
function toRecipeOperation(value: unknown): ProjectCadAutomationOperation | null {
	if (!value || typeof value !== "object") {
		return null;
	}
...
function normalizeRecipe(
	value: unknown,
): ProjectAutomationRecipeRecord | null {
	if (!value || typeof value !== "object") {
		return null;
	}
	...
	return {
		...
		steps: Array.isArray(candidate.steps)
			? candidate.steps
					...
					.filter((entry): entry is ProjectAutomationRecipeStep => entry !== null)
			: [],
		createdAt: normalizeText(candidate.createdAt) || new Date().toISOString(),
		updatedAt: normalizeText(candidate.updatedAt) || new Date().toISOString(),
		warnings: normalizeStringArray(candidate.warnings),
...
function normalizeRun(
	value: unknown,
): ProjectAutomationRunRecord | null {
	if (!value || typeof value !== "object") {
		return null;
	}
	...
	return {
		...
		verificationArtifacts: Array.isArray(candidate.verificationArtifacts)
			? candidate.verificationArtifacts
					...
					.filter(
						(entry): entry is CadVerificationArtifact => entry !== null,
					)
			: [],
		createdAt: normalizeText(candidate.createdAt) || new Date().toISOString(),
		updatedAt: normalizeText(candidate.updatedAt) || new Date().toISOString(),
...
function toRunInsert(
	...
	return {
		...
		verification_artifacts:
			record.verificationArtifacts as unknown as Json,
		user_id: userId,
...
function sortByUpdatedAtDescending<T extends { updatedAt?: string; createdAt?: string }>(
	entries: T[],
) {
...
function readLocalRecords<T extends { updatedAt?: string; createdAt?: string }>(
	...
	try {
		const raw = localStorage.getItem(buildLocalStorageKey(projectId, suffix));
		if (!raw) {
			return [] as T[];
		}
...
function writeLocalRecords<T extends { updatedAt?: string; createdAt?: string }>(
	projectId: string,
	suffix: string,
	records: T[],
) {
...
async function loadRecords<T extends { updatedAt?: string; createdAt?: string }>(
	projectId: string,
	suffix: string,
	settingKey: string,
	normalizeRecord: (value: unknown) => T | null,
	...
	try {
		...
		return {
			...
			error:
				error instanceof Error
					? error
					: new Error(`Unable to load ${suffix}.`),
		};
	}
}
...
class ProjectAutomationRecipeService {
	...
	async saveWorkPackage(
		record: ProjectAutomationWorkPackageRecord,
	): Promise<{
	...
	async saveRecipe(
		record: ProjectAutomationRecipeRecord,
	): Promise<{
	...
	async fetchRuns(projectId: string) {
		...
		return loadRecords(normalizedProjectId, "runs", RUN_SETTING_KEY, normalizeRun);
	}
			...
			return {
				...
				steps: Array.isArray(body?.steps)
					? body.steps
							...
							.filter(
								(entry: ProjectAutomationRecipeStep | null): entry is ProjectAutomationRecipeStep =>
									entry !== null,
							)
					: [],
				operations: Array.isArray(body?.operations)
					? body.operations
							...
							.filter(
								(entry: ProjectCadAutomationOperation | null): entry is ProjectCadAutomationOperation =>
									entry !== null,
							)
					: [],
				warnings: normalizeStringArray(body?.warnings),
	...
	async applyRecipe(
		...
		try {
			...
			if (!body?.success) {
				throw new Error(
					String(body?.error || body?.message || "Automation recipe apply failed."),
				);
			}
			...
			return {
				...
				artifacts: Array.isArray(body?.artifacts)
					? body.artifacts
							...
							.filter(
								(entry: CadVerificationArtifact | null): entry is CadVerificationArtifact =>
									entry !== null,
							)
					: [],
				operations: Array.isArray(body?.operations)
				...
				message:
					normalizeText(body?.message) ||
					"Automation recipe apply completed.",
			};
		} catch (error) {
			throw new Error(
	...
	async verifyRecipe(
		...
		try {
			...
			if (!body?.success) {
				throw new Error(
					String(body?.error || body?.message || "Automation recipe verify failed."),
				);
			}
			const runId = normalizeText(body?.runId);
			if (!runId) {
	...
	async reconcileAcadeProjectScope(
		...
		try {
			...
			return {
				...
				warnings: normalizeStringArray(body?.warnings),
				message:
					normalizeText(body?.message) || "ACADE reconcile completed.",
			};
		} catch (error) {
