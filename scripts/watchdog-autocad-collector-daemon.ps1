...
$runnerScript = (Resolve-Path (Join-Path $PSScriptRoot "run-watchdog-autocad-state-collector.py")).Path
...
if (-not $MutexName) {
    $MutexName = "Local\SuiteWatchdogAutoCADCollectorDaemon-$($identity.WorkstationId)"
}

$logDir = Join-Path $localAppData "Suite\watchdog-autocad-collector\logs"
...
try {
    ...
    if (-not $hasHandle) {
        Write-DaemonLog "Existing AutoCAD daemon instance detected for $($identity.WorkstationId); exiting."
        return
    }

    Write-DaemonLog "AutoCAD daemon started for workstation $($identity.WorkstationId). Config path: $ConfigPath"
    ...
    while ($true) {
        ...
        try {
            ...
            Write-DaemonLog "Launching AutoCAD collector."
            ...
            Write-DaemonLog "AutoCAD collector exited with code $exitCode."
        }
        catch {
            Write-DaemonLog "AutoCAD collector failed: $($_.Exception.Message)"
        }
