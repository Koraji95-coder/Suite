...
const studioContext: AutomationStudioContext = {
	...
	drawingRootPath: "C:/Projects/Nanulak/Drawings",
	watchdogRootPath: "C:/Projects/Nanulak",
	pdfPackageRootPath: "C:/Projects/Nanulak/PDF",
};
...
const defaultPreviewResult = {
	...
	operations: [
		{
			...
			drawingPath: "C:/Projects/Nanulak/Drawings/Issued/A-100.dwg",
			drawingName: "A-100.dwg",
	...
	warnings: [],
	blockers: [],
	message: "Recipe preview built 2 CAD operation(s) across 1 scoped drawing(s).",
};
...
function renderPanel(args?: {
	...
	const issueSet =
		args && "selectedIssueSet" in args ? args.selectedIssueSet : selectedIssueSet;
	return render(
...
describe("AutomationRecipePanel", () => {
	beforeEach(() => {
		...
		recipePanelMocks.reconcileAcadeProjectScopeMock.mockResolvedValue({
			requestId: "acade-1",
			drawingCount: 1,
			acadeProjectFilePath: "C:/Projects/Nanulak/demo.wdp",
			acadeSupportFiles: ["C:/Projects/Nanulak/demo.wdp"],
			blockers: [],
			warnings: [],
	...
	it("requires an issue set before enabling the shared runner", () => {
		...
		expect(
			screen.getByText(/Select an issue set before building a shared offline work package/i),
		).toBeTruthy();
	});
	...
	it("saves a work package and runs preflight against the selected issue set", async () => {
		...
		await waitFor(() => {
			expect(
				(screen.getByRole("button", { name: /Run preflight/i }) as HTMLButtonElement)
					.disabled,
			).toBe(false);
		});
	...
	it("previews and applies a combined recipe, then records a receipt", async () => {
		...
		await waitFor(() => {
			expect(
				(screen.getByRole("button", { name: /Preview recipe/i }) as HTMLButtonElement)
					.disabled,
			).toBe(false);
		});
		...
		await waitFor(() => {
			expect(
				(screen.getByRole("button", { name: /Apply recipe/i }) as HTMLButtonElement)
					.disabled,
			).toBe(false);
		});
