...
function renderFormattedContent(
	content: string,
	palette: { primary: string; surfaceLight: string },
) {
	...
	if (lastIndex < content.length) {
		parts.push(
			...renderBoldSegments(content.slice(lastIndex), parts.length, palette),
		);
	}
...
function renderBoldSegments(
	...
	while ((match = boldRegex.exec(text)) !== null) {
		...
		parts.push(
			<strong key={`bold-${keyOffset}-${match.index}`}>{match[1]}</strong>,
		);
		lastIndex = match.index + match[0].length;
	}
		...
		>
			...
			>
				{messages.map((msg) => {
					...
					const isAgentTask = isUser && msg.content.startsWith("[Agent Task]");

					// Parse agent task message
					let displayContent = msg.content;
					let taskName = "";
					if (isAgentTask) {
						const lines = msg.content.split("\n");
						taskName = lines[0].replace("[Agent Task] ", "").trim();
						displayContent = lines.slice(2).join("\n"); // Skip marker line and blank line
					}
							...
							<div
								style={{
									...
									borderRadius: isUser
										? "14px 14px 4px 14px"
										: "14px 14px 14px 4px",
									background: isAgentTask
										? hexToRgba("#f59e0b", 0.1) // amber background for agent tasks
										: isUser
									...
									border: `1px solid ${hexToRgba(
										...
										isAgentTask ? 0.3 : isUser ? 0.25 : 0.6,
									)}`,
									color: palette.text,
									fontSize: 13,
									lineHeight: 1.55,
