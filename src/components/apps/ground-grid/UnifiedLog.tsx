...
import { useGroundGrid, type LogEntry } from './GroundGridContext';
...
function getMessageColor(msg: string, palette: { textMuted: string; primary: string }): string {
...
export function UnifiedLog() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: 16, gap: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: palette.text }}>
          Unified Log
          <span style={{ fontSize: 11, fontWeight: 400, color: palette.textMuted, marginLeft: 8 }}>
            {logs.length} entries
          </span>
        </div>
        <button
      ...
      <div
        ref={scrollRef}
        style={{
          flex: 1, overflow: 'auto', padding: 12, borderRadius: 6,
      ...
      >
        {logs.length === 0 ? (
          <div style={{ color: palette.textMuted, textAlign: 'center', padding: 40 }}>
            No log entries yet. Activity from Coordinate Grabber and Grid Generator will appear here.
          </div>
        ) : (
          logs.map((entry, idx) => (
            <div
            ...
            >
              <span style={{ color: palette.textMuted, flexShrink: 0 }}>{entry.timestamp}</span>
              <span
                style={{
