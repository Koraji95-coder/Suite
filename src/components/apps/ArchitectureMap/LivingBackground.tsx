...
function Tendrils({
	...
	const tubeGeo = useMemo(() => {
		...
		for (let i = 0; i <= seg; i++) {
			...
			pts.push(
				new THREE.Vector3(Math.cos(a) * rad, Math.sin(a) * rad * 0.7, z),
			);
		}
	...
	const updateInstances = useCallback(
		(time: number) => {
			...
			for (let i = 0; i < seeds.length; i++) {
				...
				const y =
					c.y +
					Math.sin(s.angle * 2 + wave2) * r * 0.28 +
					bounds.radius * 0.08 * s.heightFactor;
				const z =
					c.z + Math.sin(s.angle + wave1) * r * 0.42 - bounds.radius * 0.65;
				...
				const pulse =
					thicknessPulse > 0
						? 1 +
							Math.sin(time * thicknessPulseSpeed + s.offset) * thicknessPulse
						: 1;
	...
	return (
		<instancedMesh
			ref={instancedRef}
			args={[tubeGeo, undefined as unknown as THREE.Material, count]}
			frustumCulled={false}
		>
...
function LivingParticles({
	...
	const geom = useMemo(() => {
		...
		return g;
	}, [bounds.center, bounds.radius, count, palette]);
	...
	return (
		<mesh
			...
			position={[
				bounds.center.x,
				bounds.center.y,
				bounds.center.z - bounds.radius * 0.6,
			]}
		>
