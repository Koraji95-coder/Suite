 ...
 */
export async function saveToDisk(yamlStr: string, filename?: string): Promise<{ ok: boolean; filename?: string }> {
  const name = filename ?? `suite_backup_${new Date().toISOString().replace(/[:.]/g, '-')}.yaml`;
  try {
...
export async function readBackupFile(filename: string): Promise<string | null> {
  try {
    const res = await fetch(`/api/backup/read?file=${encodeURIComponent(filename)}`);
...
export async function deleteBackupFile(filename: string): Promise<boolean> {
  try {
    const res = await fetch(`/api/backup/delete?file=${encodeURIComponent(filename)}`, { method: 'DELETE' });
    return res.ok;
  } catch {
    return false;
  }
 ...
 */
export async function restoreFromYaml(yamlStr: string): Promise<{ restored: number; errors: string[] }> {
  ...
  const orderedTables: BackupTable[] = [
    'formulas', 'saved_calculations', 'saved_circuits', 'user_preferences',
    'projects', 'tasks', 'files', 'activity_log', 'calendar_events',
    'whiteboards', 'ai_conversations', 'ai_memory', 'block_library',
    'automation_workflows', 'drawing_annotations',
  ];
