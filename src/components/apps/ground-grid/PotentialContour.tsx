  ...
  for (let j = 0; j < RESOLUTION; j++) {
    ...
    for (let i = 0; i < RESOLUTION; i++) {
      ...
      for (const c of conductors) {
        ...
        const t = Math.max(0, Math.min(1, ((px - c.x1) * dx + (py - c.y1) * dy) / (len * len)));
        const closestX = c.x1 + t * dx;
        const closestY = c.y1 + t * dy;
        const dist = Math.max(Math.sqrt((px - closestX) ** 2 + (py - closestY) ** 2), 0.3);
...
export function PotentialContour({ rods, conductors, soilResistivity, faultCurrent }: PotentialContourProps) {
  ...
  if (!hasData) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: 300, color: palette.textMuted, fontSize: 13 }}>
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', width: '100%', height: '100%', minHeight: 400 }}>
      ...
      {hoveredValue && (
        ...
        >
          <div>Position: ({hoveredValue.x.toFixed(1)}, {hoveredValue.y.toFixed(1)})</div>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <div style={{ width: 60, height: 8, borderRadius: 2, background: 'linear-gradient(90deg, #228b22, #3b82f6, #eab308, #f59e0b, #ef4444)' }} />
        </div>
