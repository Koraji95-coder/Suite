...
import { Clock, Settings2, Sparkles } from "lucide-react";
...
const LiveClockBadge = memo(function LiveClockBadge() {
	...
	return <span className="text-lg font-mono">{now.toLocaleTimeString()}</span>;
});
...
export function DashboardOverviewPanel({
	...
	onCalendarMonthChange: _onCalendarMonthChange,
	...
	// eslint-disable-next-line @typescript-eslint/no-unused-vars
	const [internalCalendarMonth, _setInternalCalendarMonth] = useState(
		new Date(),
	);
	...
	void (externalMonth ?? internalCalendarMonth);
		...
		try {
			...
			const [projectsResult, activitiesResult, filesResult] = await Promise.all(
				[
					...
					safeSupabaseQuery(
						async () => await supabase.from("files").select("size"),
						"MainDashboard",
					),
				],
			);
	...
	const handleNavigateToProjectsHub =
		onNavigateToProjectsHub ?? (() => navigate("/projects"));
