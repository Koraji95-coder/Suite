...
} from "./automationStudioSelectors";
import type {
...
import { useAutomationStudioState } from "./useAutomationStudioState";
...
export function AutomationStudioApp({
	...
	const handleRecordReceipt = async () => {
		...
		const markupFamilyCounts =
			(autoDraftSnapshot?.previewOperations ?? []).reduce(
				(counts, operation) => {
		...
		const result = await projectAutomationReceiptService.saveReceipt({
			...
			terminalScheduleSnapshotId:
				autoWireSnapshot?.scheduleSnapshotId ?? null,
			reportId:
				autoWireSnapshot?.reportId ??
				cadUtilitySnapshot?.reportId ??
				null,
