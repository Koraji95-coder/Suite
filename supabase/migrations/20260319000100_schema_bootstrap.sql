-- Local Supabase migration source of truth for Suite schema bootstrap.
-- Legacy hosted fallback copy remains at supabase/consolidated_migration.sql.
-- Follow with:
...
create table if not exists public.drawing_revision_register_entries (
	...
	revision_description text not null default '',
	revision_by text not null default '',
	revision_checked_by text not null default '',
	revision_date date null,
	revision_sort_order integer not null default 0,
...
create index if not exists idx_project_title_block_profiles_project_id on public.project_title_block_profiles (project_id);
create index if not exists idx_project_title_block_profiles_user_id on public.project_title_block_profiles (user_id, updated_at desc);
