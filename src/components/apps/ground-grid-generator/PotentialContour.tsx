// src/components/apps/ground-grid/PotentialContour.tsx
...
export function PotentialContour({
	...
	useEffect(() => {
		...
		// keep CSS sizing stable, set backing store for crispness
		...
		ctx.lineWidth = 1.5;
		for (const c of conductors) {
			const x1 = ((c.x1 - bounds.minX) / spanX) * cssW;
			const y1 = ((c.y1 - bounds.minY) / spanY) * cssH;
			const x2 = ((c.x2 - bounds.minX) / spanX) * cssW;
			const y2 = ((c.y2 - bounds.minY) / spanY) * cssH;
			ctx.beginPath();
			ctx.moveTo(x1, y1);
			ctx.lineTo(x2, y2);
			ctx.stroke();
		...
		for (const r of rods) {
			const rx = ((r.grid_x - bounds.minX) / spanX) * cssW;
			const ry = ((r.grid_y - bounds.minY) / spanY) * cssH;
