...
import {
	...
	type TitleBlockRevisionDisplayRow,
...
function readCellText(row: ExcelJS.Row, index: number) {
...
export const projectDocumentMetadataService = {
	async loadSnapshot(args: {
		...
		const [profileResult, revisionsResult] = await Promise.all([
			projectTitleBlockProfileService.fetchProfile(projectId, {
				projectRootPath,
			}),
		...
		const profile = {
			...
			signerDrawnBy: profileResult.data.signer_drawn_by,
			signerCheckedBy: profileResult.data.signer_checked_by,
			signerEngineer: profileResult.data.signer_engineer,
		} satisfies TitleBlockSyncProfile;

		const scanResponse = await titleBlockSyncService.scan({
			projectId,
			projectRootPath,
			profile,
		...
		if (!scanResponse.success || !scanResponse.data) {
			throw new Error(scanResponse.message || "Project metadata scan failed.");
		}
		...
		return {
			projectId,
			projectRootPath: scanResponse.data.projectRootPath,
			profile: scanResponse.data.profile,
			summary: scanResponse.data.summary,
			artifacts: scanResponse.data.artifacts,
			titleBlockRows: scanResponse.data.drawings,
