...
export function GraphInspector({ selectedNode, onClose, onDeleteMemory }: GraphInspectorProps) {
  ...
  return (
    <div style={panelStyle}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button
          onClick={onClose}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: palette.textMuted, padding: 4 }}
        >
          <X size={16} />
        </button>
      </div>

      <h3 style={{ margin: 0, fontSize: 16, fontWeight: 600, color: palette.text }}>
        {selectedNode.label}
      </h3>

      <div style={{ fontSize: 11, color: palette.textMuted, textTransform: 'uppercase', letterSpacing: 0.5 }}>
      ...
      {isMemory && typeof selectedNode.data?.strength === 'number' && (
        <div>
          ...
          <div style={{ height: 6, borderRadius: 3, background: hexToRgba(palette.textMuted, 0.15), overflow: 'hidden' }}>
            <div style={{
      ...
      {Array.isArray(selectedNode.data?.connections) && (selectedNode.data!.connections as string[]).length > 0 && (
        <div>
          <div style={{ fontSize: 11, color: palette.textMuted, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 4 }}>
