...
export function WhiteboardCanvas({
	...
	const redrawCanvas = useCallback(() => {
		...
		actions.forEach((action) => {
			...
			if (action.tool === "pen" && action.points && action.points.length > 1) {
				...
				ctx.beginPath();
				const radius =
					Math.sqrt(action.size.width ** 2 + action.size.height ** 2) / 2;
				ctx.arc(
		...
		if (currentAction) {
			...
			if (
				...
				const radius =
					Math.sqrt(
						currentAction.size.width ** 2 + currentAction.size.height ** 2,
					) / 2;
				ctx.arc(
	...
	const stopDrawing = () => {
		if (currentAction && isDrawing) {
			if (tool === "pen" || tool === "eraser") {
				...
				if (
					currentAction.size &&
					(currentAction.size.width !== 0 || currentAction.size.height !== 0)
				) {
					onActionAdd(currentAction);
				}
			}
	...
	return (
		<canvas
			...
			className="cursor-crosshair rounded-lg border border-[var(--color-border)] shadow-2xl"
		/>
	);
}
